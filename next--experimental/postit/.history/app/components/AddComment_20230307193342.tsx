...
export default function AddComment({ id }: ID) {
    ...
    const submitComment = async (e: React.FormEvent) => {
        ...
        if (title.length > 300) return toast.error("are u crazy? it's too much", { id: toastCommentID })
        if (title.length < 1) return toast.error('write comment please mf:)!', { id: toastCommentID })
        setIsDisabled(true)
        mutate(title, id)
    }
    return (
